...
class BasicMemoryTestCase : public ::testing::Test {
protected:
    ShaderVerificationEnv* env;
    MemorySubsystemTest* test;
    ...
    void SetUp() override {
        ...
        // Create test case
        test = new MemorySubsystemTest("basic_memory_test");
        
        // Configure environment
        VerificationConfig config;
        config.max_cycles = 300;
        config.trace_enabled = true;
        config.trace_file = "basic_memory_test.vcd";
...
TEST_F(BasicMemoryTestCase, MemoryTransactions) {
    // Run the test using the verification environment
    bool result = env->run_test(test);
    
    // Check result
    EXPECT_TRUE(result) << "Memory test failed";
